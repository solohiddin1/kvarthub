...
const Payment = () => {
  ...
  const handleUpdateStatus = (cardId: number, currentStatus: boolean) => {
    ...
    apiClient
      ...
      .then((res) => {
        ...
        setPayments(prev => prev.map(card => 
  ...
  const handleAddCard = (e: React.FormEvent) => {
    ...
    if (!newCard.card_number || !newCard.card_holder_name || 
        !newCard.expiry_month || !newCard.expiry_year) {
    ...
    return (
      <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 p-4 md:p-6">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-3xl font-bold text-gray-800 mb-8">Mening Kartalarim</h1>
    ...
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-800">Mening Kartalarim</h1>
            <p className="text-gray-600 mt-2">Barcha qo'shilgan bank kartalaringiz</p>
          </div>
          <button 
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-800">
                  {payments.reduce((sum, card) => sum + (parseFloat(card.balance) || 0), 0)
                    .toLocaleString("uz-UZ", { minimumFractionDigits: 2 })} UZS
                </p>
              </div>
            </div>
          </div>
        ...
        {!payments || payments.length === 0 ? (
          <div className="bg-white rounded-3xl p-12 text-center shadow-lg border border-gray-200">
            <div className="max-w-md mx-auto">
              ...
              <h3 className="text-2xl font-semibold text-gray-800 mb-3">Kartalar Topilmadi</h3>
              <p className="text-gray-600 mb-8">Hozircha hech qanday bank kartasi qo'shilmagan. Yangi karta qo'shish uchun quyidagi tugmani bosing.</p>
              <button 
                onClick={() => setShowAddCardModal(true)}
            ...
            {payments.map((card) => {
              ...
              return (
                <div key={card.id} className="group">
                  {/* Karta Dizayni */}
                  <div className={`relative bg-linear-to-br ${gradientClass} rounded-3xl p-6 shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 min-h-[220px]`}>
                    
                    {/* Chip */}
                    <div className="absolute top-6 right-6">
                      <div className="w-12 h-9 bg-linear-to-r from-yellow-400 to-yellow-300 rounded-lg flex items-center justify-center">
                    ...
                    {/* Card Logo */}
                    <div className="absolute top-6 left-6 text-white font-bold text-lg tracking-wider bg-white/20 px-3 py-1 rounded-lg">
                      {cardType.toUpperCase()}
                    </div>

                    {/* Card Number */}
                    <div className="absolute top-20 left-6 right-6">
                      <div className="text-white text-2xl tracking-wider font-semibold">
                    ...
                    <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end">
                      <div>
                        <p className="text-gray-200 text-xs uppercase tracking-wider">Karta egasi</p>
                        <p className="text-white text-lg font-semibold tracking-wider">
                          {(card.card_holder_name || "").toUpperCase()}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-gray-200 text-xs uppercase tracking-wider">Amal qilish muddati</p>
                        <p className="text-white text-lg font-semibold tracking-wider">
                    ...
                    <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2">
                      <div className={`flex items-center ${card.is_active ? 'bg-green-500' : 'bg-red-500'} text-white text-xs px-3 py-1 rounded-full`}>
                  ...
                  {/* Karta Ma'lumotlari Panel */}
                  <div className="mt-4 bg-white rounded-2xl p-5 shadow-lg border border-gray-200">
                    <div className="flex justify-between items-center mb-3">
                      ...
                      <div className={`px-3 py-1 rounded-full ${card.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'} text-sm font-medium`}>
                        {card.is_active ? 'Faol' : 'Nofaol'}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-4">
                      <div>
                        <p className="text-xs text-gray-500">Karta turi</p>
                        <p className="text-sm font-medium text-gray-800">
                          {cardType}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500">Oxirgi 4 raqam</p>
                        <p className="text-sm font-medium text-gray-800">
                          {card.card_number_last4 || 'Noma\'lum'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-4">
                      <div>
                        <p className="text-xs text-gray-500">Qo'shilgan sana</p>
                        <p className="text-sm font-medium text-gray-800">
                          {card.created_at ? new Date(card.created_at).toLocaleDateString("uz-UZ") : "Noma'lum"}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500">Yangilangan</p>
                        <p className="text-sm font-medium text-gray-800">
                          {card.updated_at ? new Date(card.updated_at).toLocaleDateString("uz-UZ") : "Noma'lum"}
                        </p>
                      </div>
                    </div>

                    {/* Action Buttons */}
                    <div className="flex space-x-2 mt-5 pt-4 border-t border-gray-100">
                      <button 
                        onClick={() => {
                          setSelectedCard(card);
                          setShowStatusModal(true);
                        }}
                        className="flex-1 px-4 py-2 bg-blue-50 text-blue-600 font-medium rounded-xl hover:bg-blue-100 transition-all duration-200 text-sm"
                      >
                        {card.is_active ? 'Faolsizlashtirish' : 'Faollashtirish'}
                      </button>
                      ...
                      <button 
                        ...
                        className="px-4 py-2 bg-red-50 text-red-600 font-medium rounded-xl hover:bg-red-100 transition-all duration-200 text-sm"
                      >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-800">Yangi Karta Qo'shish</h2>
              <button 
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewCard({...newCard, card_number: formatCardNumberInput(e.target.value)})}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full">
            <div className="text-center mb-6">
              <div className={`w-16 h-16 mx-auto rounded-full flex items-center justify-center ${selectedCard.is_active ? 'bg-red-100' : 'bg-green-100'} mb-4`}>
                <svg className={`w-8 h-8 ${selectedCard.is_active ? 'text-red-600' : 'text-green-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <h2 className="text-2xl font-bold text-gray-800 mb-2">
                {selectedCard.is_active ? 'Kartani faolsizlashtirish?' : 'Kartani faollashtirish?'}
              </h2>
            ...
            <div className="flex space-x-3">
              ...
              <button
                onClick={() => handleUpdateStatus(selectedCard.id, selectedCard.is_active)}
                className={`flex-1 px-4 py-3 ${selectedCard.is_active ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'} text-white font-medium rounded-xl transition-all disabled:opacity-50`}
                disabled={isProcessing}
              >
                {isProcessing ? 'Kutilmoqda...' : (selectedCard.is_active ? 'Faolsizlashtirish' : 'Faollashtirish')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full">
            <div className="text-center mb-6">
              ...
              <h2 className="text-2xl font-bold text-gray-800 mb-2">Kartani o'chirish?</h2>
              <p className="text-gray-600">
                Ushbu kartani o'chirishga ishonchingiz komilmi? Agar bu karta bilan bog'liq faol e'lonlar bo'lsa, ular ham o'chiriladi. Bu amalni teskari qilib bo'lmaydi.
              </p>
            </div>
