...
useEffect(() =>{
  apiClient.get("/api/shared/regions/").then(res =>{
    if(listing?.region){
      const regionName:RegionsType = res.data.result.find((item:RegionsType) => item.id === listing?.region)
      if(regionName){
...
useEffect(() =>{
  ...
  apiClient.get("/api/shared/districts/").then(res =>{
    if(res.data.result.length > 0){
      const DistrictsName:DistrictType = res.data.result.find((item:DistrictType) => item.id === listing?.district)
    ...
    <div className="space-y-4">
      ...
      <div className="relative overflow-hidden rounded-2xl shadow-lg group">
        ...
        <div className="absolute top-4 left-4">
          <span className={`px-4 py-2 rounded-full text-sm font-semibold shadow-lg ${listing.is_active
      ...
      {listing.images && listing.images.length > 1 && (
        <div className="flex gap-3 overflow-x-auto pb-2">
          {listing.images.map((img, idx) => (
            ...
            >
              <div className={`absolute inset-0 rounded-xl transition-all duration-300 ${idx === selectedImageIndex
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-2">
        <div className="inline-block bg-linear-to-r from-blue-50 to-indigo-50 px-4 py-2 rounded-full mb-3">
          <span className="text-blue-600 font-semibold text-sm">🏠 Uy joy</span>
        </div>
      ...
      <div className="bg-linear-to-r from-green-50 to-emerald-50 p-6 rounded-2xl border border-green-100">
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <span className="text-emerald-700 font-medium">To'lov faqat oylik asosida</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div className="bg-linear-to-br from-blue-50 to-white p-5 rounded-xl border border-blue-100 text-center">
          ...
          <p className="text-2xl font-bold text-gray-900">{listing.rooms} ta</p>
        </div>
        ...
        <div className="bg-linear-to-br from-amber-50 to-white p-5 rounded-xl border border-amber-100 text-center">
          ...
          <p className="text-lg font-bold text-gray-900 truncate">{district}</p>
        </div>
      </div>
      ...
      <div className="bg-linear-to-r from-gray-50 to-gray-100 p-6 rounded-2xl">
        ...
        <p className="text-gray-800 text-lg font-medium mb-6">{listing.location}</p>
          ...
          <div className="flex items-center justify-between p-4 bg-white rounded-xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-gray-900">{listing.phone_number}</p>
              </div>
            </div>
